...
function App() {
  const submitHandler = (event) => {
    ...
    setTitle("");
    setDescription("");
    setMainTasks([...mainTasks, { title: title, description: description }]);
...
const deletehandler=(index)=>{
  ...
  if (mainTasks.length > 0) {
   renderTask = mainTasks.map((task, index) => (
      <div key={index} className="flex justify-between p-4 bg-white rounded-lg shadow-md my-2">
        <div>
          <h4 className="text-lg font-semibold text-gray-800">{task.title}</h4>
          <h5 className="text-gray-600">{task.description}</h5>
        </div>
        <button className="text-white-700 p-2 px-3 rounded-md h-fit bg-red-400 hover:font-bold hover:bg-red-600" onClick={()=>{
          deletehandler(index)
        }}>delete</button>
      </div>
    ));
